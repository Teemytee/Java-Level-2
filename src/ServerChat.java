...
public class ServerChat implements Chat {
    ...
    public void privateMessage(StringBuilder message, String receiver, String sender){
        for (ClientHandler client : clients) {
            if (client.getName().equals(receiver)){
                client.sendMessage(String.format("[%s] sent you a private message: %s", sender, message));
            }
        }
    }

    @Override
    public void errorMessage(int errorNumber, String nickname) {
        if (errorNumber == 1){
            for (ClientHandler client : clients) {
